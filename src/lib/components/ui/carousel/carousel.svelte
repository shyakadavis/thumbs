<script lang="ts">
	...
	////
	const scroll_snaps_store = writable<number[]>([]);
	const selected_index = writable(0);
	...
	function onSelect(api: CarouselAPI) {
		...
		selected_index.set(api.selectedScrollSnap());
	}
	...
	setEmblaContex({
		...
		onInit,
		////
		scroll_snaps: scroll_snaps_store,
		selected_index,
		scrollTo
	});
	...
	function onInit(event: CustomEvent<CarouselAPI>) {
		...
		apiStore.set(api);
		////
		scroll_snaps_store.set(api.scrollSnapList());
	}
